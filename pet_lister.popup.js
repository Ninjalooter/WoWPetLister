(function () {
	'use strict';
	function PopupModel(region, realm, character, spellList, spell) {
		var selectedSpell = spell;
		var that = this;

		// ------ NAV ----------------
		that.selectNextSpell = function () {
			var next = false;
			jQuery.each(spellList, function (i, spell) {
				if (next) {
					selectedSpell = spell;
					next = false;
				} else if (spell.id === selectedSpell.id) {
					next = true;
				}
			});
		};
		that.selectPreviousSpell = function () {
			jQuery.each(spellList, function (index, spell) {
				if (spell.id === selectedSpell.id && index > 0) {
					selectedSpell = spellList[index - 1];
				}
			});
		};

		// ----- GETTER --------------
		that.getSpell = function () {
			return selectedSpell;
		};
		that.getRegion = function () { return region; };
		that.getRealmName = function () {
			return realm;
		};
		that.getCharacter = function () { return character; };

		// --------- AJAX ----------------------
		that.fetchMonthlyAuctions = function (itemId, callback) {
			jQuery.ajax('http://api.ahspy.com/' + region.substr(0, 2) + '/' + realm + '/item/' + itemId + '/monthly.json', {
				jsonp: 'callback',
				dataType: 'jsonp',
	...
	function PopupView() {
		...
		that.init = function () {
			...
			$('.popup_nav', that.popupWindow).click(function (event) {
		...
		// Event Handlers:
		that.onNavPrevious = function () {
			// Overwrite elsewhere
		};
		that.onNavNext = function () {
			// Overwrite elsewhere
		};
		...
		that.formatPrice = function (n) {
			...
			// Build output
			if (gold > 0) {
				result += "<span class=icon-gold>" + gold + "</span>";
			}
			if (silver > 0) {
				result += "<span class=icon-silver>" + silver + "</span>";
			}
			if (copper > 0) {
				result += "<span class=icon-copper>" + copper + "</span>";
			}
			return result;
		};
	...
	function PopupPresenter(view, model) {
		var that = this;
		that.onSpellChange = function () {
			var spell = model.getSpell();
			...
			if (model.isAHSpyRegion()) {
				// Show header
				view.showAHTableHeader(model.getRealmName());
				var faction = $.battlenet({action: 'get-faction-by-race', race:model.getCharacter().race}).toLowerCase();
				...
				jQuery.each(itemIds, function (i, itemId) {
					view.showAHLoadingTable(
						itemId, 
						ahspy_itemurl(model.getRegion(), model.getRealmName(), faction, itemId)
					);
					...
					model.fetchMonthlyAuctions(itemId, function (err, data) {
						if ( (data.a == null && data.h == null && data.n == null) ||
							...
							view.showAHPriceTable(itemId, data);
						}
					});
				});
			} else {
		...
		view.onNavNext = function () {
			model.selectNextSpell();
			view.reset();
			that.onSpellChange();
		};
		view.onNavPrevious = function () {
			model.selectPreviousSpell();
			view.reset();
			that.onSpellChange();
		};
		return that;
	...
	window.popup_show = function (character, spellList, spell)
	{	
		var region = $("#region").val(); // eu, us, kr, ...
		var realm = $("#realm").val();
		
		var view = PopupView();
		var model = new PopupModel(region, realm, character, spellList, spell);
		var presenter = new PopupPresenter(view, model);
		presenter.onSpellChange();
	}
	/*
		...
		if ( state == 1) {
			window.setTimeout(function() {
				view.showAHPriceTable(66096, {
					...
					'n': { 'qty': '1', 'min': '125123', 'max': '15125123', 'avg': '129923'}
				});
